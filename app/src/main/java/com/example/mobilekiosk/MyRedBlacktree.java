...
class MyRedBlacktree{
    ...
    treenode Treesearch(String Num) {
    ...
    treenode insert(MyRedBlacktree tree,String data, String st,String id,OrderData []OrderList, String DBroot){
        if(tree.root==null){

            tree.root=new treenode(data,st,id,OrderList,DBroot);
            if(tree.root==null)
                return null;

        }
        else{
            treenode temp=new treenode("",st,id,OrderList,DBroot);   //A temporary root
            ...
            while(true){
                ...
                if(q==null){
                    //Inserting root node
                    q=new treenode(data,st,id,OrderList,DBroot);
                    p.children[dir]=q;
...
class treenode{
    ...
    String st;   //가계명
    String id;   //유저 아이디
    OrderData []OrderList; //메뉴리스트
    String color;
    treenode children[];
    String DBroot;

    public treenode(String data, String st,String id,OrderData []OrderListm, String DBroot){
        this.data = data;
        this.st=st;
        this.id=id;
